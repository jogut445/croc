/*
	...
	- Added DIN_DELAY parameter to FLASH_READER_QSPI and EF_QSPI_XIP_CTRL to
	  compensate for GPIO input synchronizer latency (floor(N/2) SCK cycles,
	  where N is the total number of synchronizer flip-flops).
...
*/
// DIN_DELAY: number of extra SCK cycles to defer the capture window.
// Set to floor(N/2) where N is the total number of GPIO input FFs.
// 0 = direct pin, 1 = 2-3 FF synchroniser (default), 2 = 4-5 FFs, etc.
module FLASH_READER_QSPI #(parameter LINE_SIZE   = 16,
                            parameter DIN_DELAY   = 1)(
    input   wire                        clk,
    input   wire                        rst_n,
    ...
    always @ (posedge clk)
        if(counter >= 20+DIN_DELAY && counter <= 19+DIN_DELAY+LINE_BYTES*2)
            if(sck)
                data[(counter-DIN_DELAY)/2 - 10] <= {data[(counter-DIN_DELAY)/2 - 10][3:0], din};
    ...
    assign done     = (counter == 19+DIN_DELAY+LINE_BYTES*2);
...
(
    ...
    FLASH_READER_QSPI #(.LINE_SIZE(LINE_SIZE), .DIN_DELAY(DIN_DELAY))
    READER (
        .clk(clk),
        .rst_n(rst_n),
        .addr(addr),
