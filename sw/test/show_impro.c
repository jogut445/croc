...
static uint8_t gen_max_val(void) {
    uint8_t v = 1;
    while ((uint32_t)(v + 1u) * (uint32_t)(v + 1u) <= 255u)
        v++;
...
static void gen_data(void) {
    ...
    for (int i = 0; i < N; i++)
        for (int k = 0; k < K; k++)
            A[i * K + k] = (uint8_t)((i * 3 + k * 7) % v + 1);
    for (int k = 0; k < K; k++)
        for (int j = 0; j < M; j++)
            B[k * M + j] = (uint8_t)((k * 5 + j * 2) % v + 1);
}
...
static void axpy_naive(void) {
    for (int i = 0; i < N * M; i++)
        Z_naive[i] = (uint8_t)(AXPY_ALPHA * (uint32_t)X[i] + (uint32_t)Y[i]);
}
...
static void gemm_naive(void) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            uint32_t s = 0;
            for (int k = 0; k < K; k++)
                s += (uint32_t)A[i * K + k] * (uint32_t)B[k * M + j];
            C_naive[i * M + j] = s;
        }
    }
}
...
int main(void) {
    ...
    for (int idx = 0; idx < N * M; idx++) {
        if (C_naive[idx] != C_simd[idx]) { gemm_ok = 0; break; }
    }

    int axpy_ok = 1;
    for (int idx = 0; idx < N * M; idx++) {
        if (Z_naive[idx] != Z_simd[idx]) { axpy_ok = 0; break; }
    }
