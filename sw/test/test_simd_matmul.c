...
static void matmul_scalar(const uint8_t A[2][4], const uint8_t B[4][2], uint32_t C[2][2]) {
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++) {
            C[i][j] = 0;
            for (int k = 0; k < 4; k++) C[i][j] += (uint32_t)A[i][k] * B[k][j];
        }
}

static void matmul_simd(const uint8_t A[2][4], const uint8_t B[4][2], uint32_t C[2][2]) {
    for (int i = 0; i < 2; i++) {
